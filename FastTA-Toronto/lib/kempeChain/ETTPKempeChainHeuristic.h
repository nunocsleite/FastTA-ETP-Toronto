...
public:
    ...
    /**
     * @brief getNeighborExamProximityConflicts
     * @return Return neighbor exam proximity conflicts
     */
    long getNeighborExamProximityConflicts() const;
...
protected:
    ...
    /**
     * @brief neighborSolutionCost Sum of exam proximity conflicts
     */
...
ETTPKempeChainHeuristic<EOT>::ETTPKempeChainHeuristic()
    : neighborFitness(0), neighborExamProximityConflicts(0), feasibleNeighbour(true)
{ }
...
#ifdef ETTP_KEMPE_CHAIN_HEURISTIC_DEBUG
    std::cout << "ETTPKempeChainHeuristic<EOT>::operator()(EOT & _sol)" << std::endl;
...
void ETTPKempeChainHeuristic<EOT>::apply() {
    ...
    std::vector<int> conflictingExamsTi;
    // Queue containing conflicting exams in time slot tj
    std::vector<int> conflictingExamsTj;
 ...
 */
template <typename EOT>
long ETTPKempeChainHeuristic<EOT>::getNeighborExamProximityConflicts() const {
